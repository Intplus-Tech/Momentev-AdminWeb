...
export const columns: ColumnDef<BookingResponse>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (!vendor) return <span className="text-gray-400 italic">Unknown</span>;
      return (
        <div className="flex flex-col">
          <span className="font-medium text-gray-900 capitalize">
             Vendor ({vendor._id ? vendor._id.slice(-6) : "Unknown"})
          </span>
          <span className="text-[11px] text-gray-500 truncate max-w-[150px]">
            {vendor.paymentModel ? vendor.paymentModel.replace(/_/g, " ") : "Standard"}
          </span>
        </div>
      );
    },
